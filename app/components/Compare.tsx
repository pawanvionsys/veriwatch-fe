  ...
  return (
    <section className="px-6 md:px-[60px] py-16 md:py-[80px] bg-[#0A0C10] relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        >
          {/* Features List Card */}
          <motion.div variants={cardVariants} className="bg-[#111520]/50 backdrop-blur-xl border border-white/[0.05] rounded-[32px] p-8 flex flex-col h-full">
          ...
          <motion.div 
            ...
            className="bg-[#161B26] border-2 border-[#0D9488]/40 rounded-[32px] p-8 flex flex-col h-full relative shadow-[0_0_40px_rgba(13,148,136,0.15)] transform md:-translate-y-4"
          >
            <div className="absolute -top-4 left-1/2 -translate-x-1/2 bg-[#0D9488] text-ink text-[10px] font-black uppercase tracking-widest px-4 py-1.5 rounded-full shadow-[0_10px_20px_rgba(13,148,136,0.3)]">
              Recommended
            </div>
            <h3 className="font-barlow font-bold text-white text-[28px] mb-10 flex items-center gap-3">
              <div className="w-8 h-8 rounded-lg bg-cyan/20 border border-cyan/30 flex items-center justify-center">
            ...
            <ul className="space-y-8 flex-1 mb-8">
              {features.map((f, i) => (
                <li key={i} className="flex items-center gap-3 h-6 text-[14px]">
          ...
          {/* Others Card */}
          <motion.div variants={cardVariants} className="bg-[#111520] backdrop-blur-xl border border-white/[0.05] rounded-[32px] p-8 flex flex-col h-full transition-all">
            <h3 className="font-barlow font-bold text-[#6A7A92] text-[28px] mb-10">Others</h3>
            <ul className="space-y-8 flex-1 mb-8">
              {features.map((f, i) => (
                <li key={i} className="flex items-center gap-3 h-6 text-[14px]">
                  {f.others === '✕' ? (
              ...
              {/* <li className="text-[#6A7A92] font-bold text-[24px] h-6 flex items-center pt-8 border-t border-white/[0.05] tracking-tight">
                $19.99 <span className="text-[12px] text-muted font-bold ml-2 uppercase tracking-widest">Per Month</span>
              </li> */}
            </ul>
            {/* <button disabled className="w-full py-4 rounded-2xl border border-white/10 text-[#3A4558] font-bold uppercase tracking-widest text-[12px] mt-auto">
              Switch to Better
            </button> */}
          </motion.div>
        </motion.div>
